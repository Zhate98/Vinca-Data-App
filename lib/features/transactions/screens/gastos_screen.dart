                ...
                return ListView(
                  ...
                  children: [
                    ...
                    for (final g in filtered)
                      _GastoTile(
                        gasto: g,
                        onEdit: () => _openForm(context, ref, g),
                        onDelete: () => ref
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _GastoForm(
        ...
        onSubmit: (g) => gasto == null
            ? ref.read(financeRepositoryProvider)!.addGasto(g)
            : ref.read(financeRepositoryProvider)!.updateGasto(g),
      ),
    );
  }
}

// ── Barra de filtro reutilizable ──────────────────────────────────────────────
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text('${gasto.categoria} · ${Fmt.date(gasto.fecha)}',
            style:
                const TextStyle(fontSize: 11, color: AppColors.darkMuted)),
        trailing: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('-${Fmt.money(gasto.monto)}',
                style: const TextStyle(
...
class _GastoForm extends StatefulWidget {
  const _GastoForm({required this.onSubmit, this.gasto});
...
class _GastoFormState extends State<_GastoForm> {
  ...
  late String   _persona= widget.gasto?.persona   ?? FinanceConstants.personasBase.first;
